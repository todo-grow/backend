...
class TestUserWithdrawAPI:
    def test_withdraw_unauthorized(self):
        ...
        client = TestClient(app)
        response = client.delete("/api/auth/withdraw")
    ...
    def test_withdraw_success(self, authenticated_client, db_session):
        ...
        response = authenticated_client.delete("/api/auth/withdraw")
    ...
    def test_withdraw_with_existing_todos_and_tasks(
        ...
        # 사용자 생성
        user_orm = UserFixture.create_user_orm(
            session=db_session, id=1, kakao_id="test123", nickname="테스트유저"
        )
